...
export class BaziCalculator {
  ...
  public calculateBasicAnalysis(): BasicAnalysis {
    ...
    const lifeGua = this.analysisCalculator.calculateLifeGua(
      this.year,
      this.gender
    );
    ...
    return {
      lifeGua,
      dayMaster,
      nobleman: this.analysisCalculator.calculateNobleman(
        dayMaster.element,
        dayMaster.stem
      ),
      intelligence: this.analysisCalculator.calculateIntelligence(
        dayMaster.element
      ),
      skyHorse: this.analysisCalculator.getSkyHorse(pillars.day.chinese[1]),
      peachBlossom: this.analysisCalculator.calculatePeachBlossom(
        dayMaster.element
      ),
